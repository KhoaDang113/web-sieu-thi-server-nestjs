...
export class AssignOrderService {
    constructor(
        @InjectModel(Shipper.name) private readonly shipperModel: Model<ShipperDocument>,
    ...
    async getOnlineAvailableShippers() {
        ...
        for (const s of shippers) {
            ...
            const userId = String((s.user_id as any)?._id || s.user_id);
            const [cur] = await this.redis.mget(
                `shipper:${id}:current`
            );
            const current = Number.parseInt(cur || '0', 10);
            if (current < 1)
                list.push({ id, userId, current });
        }   
        return list;
    }
    ...
    async pickShipper(excludeShipperId?: string) {
        ...
        // Filter out the excluded shipper if provided
        const candidates = excludeShipperId ? c.filter(s => s.id !== excludeShipperId.toString()) : c;
    ...
    async shipperAssignOrder(orderId: string, shipperId: string, status: string) {
        ...
        const populatedOrder = await this.orderModel.findById(orderId)
            .populate('address_id', 'full_name phone address ward district city')
            .populate('items.product_id', 'name slug image_primary unit_price');
